    ...
    <script type="text/javascript">
 		$(function(){
 			...
 			$('#gooddg').datagrid({
 			    ...
 			    columns:[[
					{field:'gid',title:'商品ID',width:100,align:'center'},      
 					...
 					{field:'quantity',title:'库存数量',width:100,align:'center'},
 					{field:'sale',title:'销量',width:100,align:'center'},
 			...
 			// 删除商品
				...
				iconCls: 'icon-search',
				handler: function(){
					$('#searchGoodWindow').dialog({
					    title: '查找商品',
					    width: 330,
					    height: 230,
					    closed: false,
					    cache: false,
					    href:'good_sub_search',
					    modal: true
					})}
			   }]
 			});
			...
			//search窗口提交action
			function submitGoodSearch()
			{				
					var a=document.getElementById('searchGname').value;
					var url='getallgood.action?gname='+a;					
					$('#gooddg').datagrid('reload',url);
					$('#searchGoodWindow').dialog('close');					
								
			}
		
 		
 		//search窗口点击确定取消查找之后
 		function searchGoodCancel()
		{
			$('#searchGoodWindow').dialog('close');
		}
